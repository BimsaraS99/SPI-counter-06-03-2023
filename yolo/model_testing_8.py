    ...
    try:
        ...
        for i in range(4):
            try:
                ...
                if i == color:
                    space = np.zeros((25, cropped_image.shape[1], 3), dtype=np.uint8)
                    space[:, :, 0] = 220   # blue channel
                    space[:, :, 1] = 220  # green channel
                    space[:, :, 2] = 220   # red channel
...
root = tk.Tk()
root.title("Stitch_AI")
root.geometry("950x600")

# Function to select image file and print its path
# Function to select image file and print its path
def select_image():
    file_path = filedialog.askopenfilename()
    main_image, img_1, img_2, img_3, detected_count, value_SPI, pos_value, pos_spi, pros_time = Find_SPI_Values(file_path)
    print("Process Time - ", pros_time)
    # Destroy previous labels
    for widget in root.winfo_children():
        if isinstance(widget, tk.Label):
            widget.destroy()

    # Create new labels for the new images
    img_pil = Image.fromarray(cv2.cvtColor(main_image, cv2.COLOR_BGR2RGB))
    img_pil_2 = Image.fromarray(cv2.cvtColor(img_1, cv2.COLOR_BGR2RGB))
    img_pil_3 = Image.fromarray(cv2.cvtColor(img_2, cv2.COLOR_BGR2RGB))
    img_pil_4 = Image.fromarray(cv2.cvtColor(img_3, cv2.COLOR_BGR2RGB))
    
    img_tk = ImageTk.PhotoImage(img_pil)
    img_tk_2 = ImageTk.PhotoImage(img_pil_2)
    img_tk_3 = ImageTk.PhotoImage(img_pil_3)
    img_tk_4 = ImageTk.PhotoImage(img_pil_4)
    
    label = tk.Label(root, image=img_tk)
    label_2 = tk.Label(root, image=img_tk_2)
    label_3 = tk.Label(root, image=img_tk_3)
    label_4 = tk.Label(root, image=img_tk_4)
    my_label = tk.Label(root, text= "Detected count -  " + str(detected_count), font=('Helvetica', 16, 'bold'))
    my_label_2 = tk.Label(root, text="SPI - " + str(value_SPI), font=('Helvetica', 46, 'bold'))
    
    label.image = img_tk
    label_2.image = img_tk_2
    label_3.image = img_tk_3
    label_4.image = img_tk_4
    
    label.place(x=20, y=70)
    label_2.place(x=550, y=70)
    label_3.place(x=550, y=183-15)
    label_4.place(x=550, y=295-30)
    my_label.place(x=550, y=350)
    my_label_2.place(relx=0.5, rely=0.8, anchor=tk.CENTER)
...
textbox = tk.Text(root, height=1.5, width=70)
textbox.place(x=200, y=20)
# Position button using grid
