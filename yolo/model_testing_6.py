...
def visualize(
  ...
  return image, start_point, end_point, seam_detetcted, start_point_1, end_point_1
...
# ------------------------------------------------------------------------------------
def seperate_and_process(image, middle_ul, middle_br, upper_left, botom_right):
    y1 = middle_ul[1] - upper_left[1]
    y2 = middle_br[1] - botom_right[1]

    img1 = image[0:y1, :]
    img2 = image[y2:, :]
    
    img_col = ['img1', 'img2']
    img_binary = [None, None]
    ...
    try:
        for j in range(2):
            for i in range(127):
                ...
                _, f = cv2.threshold(img1, thresh_value, max_value, thresh_type)
                #cv2.countNonZero(f)

            cv2.imshow('image1'+str(j), f)
    except:
        print("Bimsara")
...
def process_image(image, color_number, middle_ul, middle_br, upper_left, botom_right):
    ...
    seperate_and_process(image, middle_ul, middle_br, upper_left, botom_right)
...
image, upper_left, botom_right, seam_state, middle_ul, middle_br = visualize(image, detections)
...
for i in range(4):
    try:
        final_image, b = process_image(cropped_image, i, middle_ul, middle_br, upper_left, botom_right)
...
cv2.putText(image, text, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
cv2.putText(image, text_2, (50, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
